...
def dijkstra(graph, source):
    ...
    while unvisited:
        ...
        for unvisited_node in unvisited:
            node_dist = result.get(unvisited_node, None)

            if node_dist is not None:
                if least_distance is None or node_dist < least_distance:
                    least_distance = node_dist
        ...
        for node_neighbour in graph.neighbours[current_node]:
            if node_neighbour in unvisited:
                neighbour_calc_dist = result[current_node] + graph.distances[(current_node, node_neighbour)]

                # 3. If the calculated distance of the unvisited neighbour is less than the already known distance in result dictionary, update the shortest distance in the result dictionary.
                if node_neighbour not in result or neighbour_calc_dist < result[node_neighbour]:
                    result[node_neighbour] = neighbour_calc_dist

                    # 4. If there is an update in the result dictionary, you need to update the path dictionary as well for the same key.
                    path[node_neighbour] = current_node
